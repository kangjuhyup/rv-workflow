...
- Load at most one Superpowers skill for the current phase.
...
- Move to another Superpowers skill only after the current phase ends and the next phase independently meets its trigger.
...
- After the specification is accepted, choose only the Superpowers skill required by the active phase.
...
Use `dispatching-parallel-agents` proactively when all of the following are true:
...
| --- | --- |
| Requirements are already clear | Skip `brainstorming`. Use `planner` only if a project specification is needed. |
| Implementation order remains non-obvious after an accepted specification | `writing-plans` may be used for a genuinely multi-stage implementation sequence. |
| Two or more workstreams pass the parallelism gate | Use `dispatching-parallel-agents` and start the focused agents concurrently. |
| A real bug or failure has an unclear cause, is flaky, or produces cascading symptoms | Use `systematic-debugging`. Skip it for an obvious typo or a directly explained failure. |
| Substantive or questionable review feedback must be evaluated | Use `receiving-code-review`. Skip it for mechanical, unambiguous edits. |
| A broad or risky implementation needs an additional review gate | Use `requesting-code-review`. Skip it for localized routine changes. |
| A medium, large or high-risk change is ready for a completion claim | Use `verification-before-completion` once with fresh, scope-appropriate evidence. For small changes, run the direct check without loading this workflow. |
...
- Read only the selected Superpowers `SKILL.md`; read its references only when that skill routes to them.
- Do not invoke `using-superpowers` as a conversation starter or discovery step.
